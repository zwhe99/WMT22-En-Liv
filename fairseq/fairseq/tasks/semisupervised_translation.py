...
class SemisupervisedTranslationTask(MultilingualTranslationTask):
    ...
    def train_step(
        ...
        def forward_backward(model, samples, logging_output_key, weight):
            ...
            for k in logging_output:
                ...
                # We need to identify what type of training these loggings came from
                type=logging_output_key.split(":")[0]
                type_key=f"{type}:{k}"
                if type_key not in agg_logging_output:
                    agg_logging_output[type_key] = 0.0
                agg_logging_output[type_key] += logging_output[k]

        if self.lambda_parallel > 0.0:
            for lang_pair in self.lang_pairs:
                if lang_pair not in sample:
                    logger.warning(f"{lang_pair} parallel data is not available at update@{update_num}]. Skip it!")
                    continue
        ...
        if self.lambda_otf_bt > 0.0:
            for lang_pair in self.lang_pairs:
                ...
                if sample_key not in sample:
                    logger.warning(f"{lang_pair} bt data is not available at update@{update_num}]. Skip it!")
                    continue
        ...
        if self.lambda_denoising > 0.0:
            for lang_pair in self.lang_pairs:
                ...
                if sample_key not in sample:
                    logger.warning(f"{lang_pair} dae data is not available at update@{update_num}]. Skip it!")
                    continue
    ...
    def reduce_metrics(self, logging_outputs, criterion):
        with metrics.aggregate():
            ...
            semisup_loss_keys = ["denoising:loss", "denoising:nll_loss", "bt:loss", "bt:nll_loss"]
            for k in semisup_loss_keys:
                sample_size = sum(log.get(k.split(":")[0]+":sample_size", 0) for log in logging_outputs)
                ntokens = sum(log.get(k.split(":")[0]+":ntokens", 0) for log in logging_outputs)
                k_sum = sum(log.get(k, 0) for log in logging_outputs)
                if sample_size != 0.0 and ntokens != 0.0:
                    if "nll_loss" in k:
                        metrics.log_scalar(k.replace(":","_"), k_sum / sample_size / math.log(2), sample_size, priority=5, round=3)
                    else :
                        metrics.log_scalar(k.replace(":","_"), k_sum / ntokens / math.log(2), ntokens, priority=5, round=3)
