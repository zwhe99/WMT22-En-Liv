...
class Trainer(object):
    ...
    def build_dictionary(self):
        ...
        src_emb_norm = src_emb.norm(2, 1, keepdim=True).expand_as(src_emb)
        tgt_emb_norm = tgt_emb.norm(2, 1, keepdim=True).expand_as(tgt_emb)
        nonzero1 = src_emb_norm > 0
        nonzero2 = tgt_emb_norm > 0
        src_emb[nonzero1] /= src_emb_norm[nonzero1]
        tgt_emb[nonzero2] /= tgt_emb_norm[nonzero2]
    ...
    def procrustes(self):
        ...
        U, S, V_t = scipy.linalg.svd(M, full_matrices=False)
    ...
    def export(self):
        ...
        new_src_emb = torch.empty((src_emb.shape[0], params.tgt_emb_dim), dtype=src_emb.dtype)
        with torch.no_grad():
            for i, k in enumerate(range(0, len(src_emb), bs)):
                x = Variable(src_emb[k:k + bs])
                new_src_emb[k:k + bs] = self.mapping(x.cuda() if params.cuda else x).data.cpu()
            
            # for (sid, tid) in self.dico:
            #     new_src_emb[sid] = tgt_emb[tid]

        # write embeddings to the disk
        export_embeddings(new_src_emb, tgt_emb, params)
